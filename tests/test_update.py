...
@pytest.fixture(params=("unique", "duplicated", "extreme_duplicated"))
def mod(request: pytest.FixtureRequest) -> Literal["unique", "duplicated", "extreme_duplicated"]:
    return request.param


@pytest.fixture(params=("intersecting",))
def across(request: pytest.FixtureRequest) -> Literal["intersecting"]:
    return request.param


@pytest.fixture(params=("joint", "disjoint"))
def n(request: pytest.FixtureRequest) -> Literal["joint", "disjoint"]:
    return request.param
...
def test_update_simple(mdata: MuData, axis: Axis):
    """
...
def test_update_add_modality(rng: np.random.Generator, modalities: Mapping[str, AnnData], axis: Axis):
    ...
    for i in (-2, -1):
        ...
        assert_dtypes(mdata.obs)
        assert_dtypes(mdata.var)

        test_obsm_values = get_attrm_values(mdata, "obs", "test", some_obs_names)
        if axis == 1:
        ...
        assert (attrnames == old_attrnames.union(getattr(modalities[modnames[i]], f"{attr}_names"), sort=False)).all()
        assert (oattrnames == old_oattrnames.append(getattr(modalities[modnames[i]], f"{oattr}_names"))).all()


def test_update_delete_modality(mdata: MuData, axis: Axis):
...
def test_update_intersecting(rng: np.random.Generator, modalities: Mapping[str, AnnData], axis: Axis):
    """
    Update should work when
    - obs_names are the same across modalities,
    ...
    for m, mod in modalities.items():
        setattr(
            mod, f"{oattr}_names", [f"{m}_{oattr}{j}" if j != 0 else f"{oattr}_{j}" for j in range(mod.shape[1 - axis])]
        )
...
def test_update_after_filter_obs_adata(mdata: MuData, axis: Axis):
    ...
    true_obsm_values = get_attrm_values(mdata[keptmask], "obs", "test", some_obs_names)
...
def test_update_after_obs_reordered(mdata: MuData):
    """
